...
class PreciseCalculator {
  ...
  // 获取最大精度
  static getMaxPrecision(...args) {
    return Math.max(...args.map((num) => this.getDecimalPlaces(num)));
  }

  // 加法
  static add(a, b) {
    const precision = this.getMaxPrecision(a, b);
    const multiplier = Math.pow(10, precision);
  ...
  static multiply(a, b) {
    ...
    const multiplierA = Math.pow(10, precisionA);
    const multiplierB = Math.pow(10, precisionB);
    return (
      (Math.round(a * multiplierA) * Math.round(b * multiplierB)) /
  ...
  // 格式化（保留指定位数）
  static format(num, precision = 2) {
    return parseFloat(num.toFixed(precision));
  }
}

// 使用示例
console.log(PreciseCalculator.add(0.1, 0.2)); // 0.3
console.log(PreciseCalculator.multiply(0.1, 3)); // 0.3
console.log(PreciseCalculator.isEqual(0.1 + 0.2, 0.3)); // true
```
...
const quantity = 3;
const totalInCents = priceInCents * quantity; // 5970 分
const totalInYuan = totalInCents / 100; // 59.70 元
...
double a = 0.1;
double b = 0.2;
cout << a + b; // 0.3（可能因为编译器优化）
```

**原因**：所有使用 IEEE 754 标准的语言都有这个问题。
