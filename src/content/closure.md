...
// 在 DevTools 中可以看到：
// Closure (middle) { middleVar: "middle" }
// Closure (outer) { outerVar: "outer", outerNum: 42 }
```
...
```javascript
function createCounter() {
  let count = 0; // 1. 创建变量 count

  return function () {
    // 2. 返回的函数形成闭包，引用 count
    count++;
    return count;
  };
}

const counter1 = createCounter(); // 3. 创建第一个闭包，count = 0
const counter2 = createCounter(); // 4. 创建第二个闭包，count = 0（独立的）
...
### 实践 2：React Hooks 中的闭包

**useState 和 useEffect 中的闭包**：
...
console.log(expensiveFunction(5)); // 10（从缓存读取）
```
...
6. **最佳实践**：
   - 优先使用 ES6 模块代替闭包模块
   - React Hooks 中注意闭包陷阱（使用函数式更新）
   - 函数式编程工具（记忆化、管道等）
   - 及时清理不需要的引用

7. **调试技巧**：
   - 使用 Chrome DevTools 查看闭包作用域
   - 理解执行上下文和变量查找机制
   - 注意多个闭包之间的独立性
